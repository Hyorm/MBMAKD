...
(3) 변환된 파일과 market-basket모델링 프로그램인 Recommender(Recommender는 입력 받은 데이터를 Market-basket 모델을 활용하여 분석한다. 또한, config.property파일을 입력받아 outlier와 threshold 처리에 필요한 값들을 얻는다. Recommender는 다음 4가지 단계를 거치며 결과를 도출해 낸다.)를 통해 결론을 도출한다.<br>
